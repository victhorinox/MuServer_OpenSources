	...
	else if (aRecv->tFlag == 0 && aRecv->sFlag == 0) //equip/unequip item
	{
		...
		if (aRecv->Source > 11) { //wear equipment case
			...
			if (pMsg.TargetSlot == 0){
				if(lpObj->Inventory[0].IsItem() /*slot 0 already has item*/ && !lpObj->Inventory[1].IsItem() /*slot 1 is empty*/ && lpObj->Inventory[aRecv->Source].m_Index < GET_ITEM(6, 0) && (lpObj->Class != CLASS_DL && lpObj->Class != CLASS_DW && lpObj->Class != CLASS_FE && lpObj->Class != CLASS_SU)){
					if (lpObj->Class == CLASS_RF){
						...
						if ((CheckItemRequireClassUnick(CLASS_RF, lpObj->Inventory[aRecv->Source].m_Index) == false && CheckItemRequireClassUnick(CLASS_RF, lpObj->Inventory[0].m_Index) == true) || (CheckItemRequireClassUnick(CLASS_RF, lpObj->Inventory[aRecv->Source].m_Index) == true && CheckItemRequireClassUnick(CLASS_RF, lpObj->Inventory[0].m_Index) == false)){
							return;
						}
					}
					pMsg.TargetSlot = 1;
					...
					if (!lpObj->Inventory[0].IsItem() && lpObj->Inventory[1].IsItem()) /*slot 0 vacio - slot 1 tiene item*/{
						if (lpObj->Class == CLASS_RF){
							if ((CheckItemRequireClassUnick(CLASS_RF, lpObj->Inventory[aRecv->Source].m_Index) == false && CheckItemRequireClassUnick(CLASS_RF, lpObj->Inventory[1].m_Index) == true) || (CheckItemRequireClassUnick(CLASS_RF, lpObj->Inventory[aRecv->Source].m_Index) == true && CheckItemRequireClassUnick(CLASS_RF, lpObj->Inventory[1].m_Index) == false)){
								return;
							}
						}
					}
			...
			else if (pMsg.TargetSlot == 10 && lpObj->Inventory[10].IsItem() /*slot 10 already has item */&& !lpObj->Inventory[11].IsItem()) /*slot 11 empty*/{	
				pMsg.TargetSlot = 11;
			}else if(pMsg.TargetSlot == 8) /*slot 10 already has item */{
				if((lpObj->Map == MAP_ICARUS && lpObj->Inventory[aRecv->Source].m_Index == GET_ITEM(13,2)) || (lpObj->Map == MAP_ATLANS && (lpObj->Inventory[aRecv->Source].m_Index == GET_ITEM(13,2) || lpObj->Inventory[aRecv->Source].m_Index == GET_ITEM(13,3)))){
					return;
				}
...
bool CItemManager::IsChaosBoxHasItem(LPOBJ lpObj) // OK
{
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		if (lpObj->ChaosBox[n].IsItem() != 0)
...
bool CItemManager::IsTradeHasItem(LPOBJ lpObj) // OK
{
	for (int n = 0; n < TRADE_SIZE; n++)
	{
		if (lpObj->Trade[n].IsItem() != 0)
