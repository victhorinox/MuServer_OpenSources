...
void CCustomMove::GetMove(LPOBJ lpObj,int index) // OK
{
	...
	if(CustomMoveInfo.MinMReset != -1 && lpObj->MasterReset < CustomMoveInfo.MinMReset) //- Fix Master Reset check in Custom Move
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(818),CustomMoveInfo.MinMReset);
		return;
	}

	if(CustomMoveInfo.MaxMReset != -1 && lpObj->MasterReset > CustomMoveInfo.MaxMReset) //- Fix Master Reset check in Custom Move
	{
