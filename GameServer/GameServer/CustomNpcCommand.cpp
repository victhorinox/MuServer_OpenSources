...
bool CCustomNpcCommand::GetNpcCommand(LPOBJ lpObj,LPOBJ lpNpc) // OK
{
	...
	for(std::map<int,NPC_TYPE_INFO>::iterator it=this->m_CustomNpcCommand.begin();it != this->m_CustomNpcCommand.end();it++)
	{
		if(it->second.MonsterClass == lpNpc->Class && it->second.Map == lpNpc->Map && it->second.X == lpNpc->X && it->second.Y == lpNpc->Y)
		{
			if (it->second.Talk == 1)
			{
				gCommandManager.ManagementCore(lpObj,it->second.Command,lpNpc->Index);
			}

			//- Add CustomMove to CustomNpcCommand
			CUSTOMMOVE_INFO CustomMoveInfo;

			if(gCustomMove.GetInfoByName(it->second.Command,&CustomMoveInfo) != 0)
			{
			...
			{
				gCommandManager.ManagementCore(lpObj,it->second.Command,-1);
			}
			return 1;
		}
