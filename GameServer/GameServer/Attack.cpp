...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	//- Custom Disable Skill System

	if (gCustomDisableSkill.GetSkillDisablePlayer(skill) == true)
	...
	if (gCustomDisableSkill.GetSkillDisableGeneral(skill) == true)
	...
	if (gDuel.CheckDuel(lpObj, lpTarget) != 0)
		...
		{
			if (gCustomDisableSkill.GetSkillDisableDuel(skill) == true)
			{
				return 0;
			}
		}
	...
	if (lpObj->GuildNumber > 0 && lpObj->Guild != 0 && lpObj->Guild->WarState != 0 )
		...
		{
			if (gCustomDisableSkill.GetSkillDisableGuild(skill) == true)
				{
					return 0;
				}
		}
	...
	if(damage == 0)
	{
		...
		if(combo != 0)
		{
			...
			skill = SKILL_COMBO;
/*
			//- Combo push effect option
			if(gServerInfo.m_ComboPushEffect == 2)
			{
				gObjBackSpring2(lpTarget,lpObj,4); //Combo Effect Push Every Time
			}*/
		}
	}
	else
	...
	if(lpObj->Type == OBJECT_USER)
	{
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			{
				//if(lpObj->CsJoinSide == 0 || lpTarget->CsJoinSide == 0 || lpObj->CsJoinSide != lpTarget->CsJoinSide) // - Fix CastleSiegeDamageRate
	...
	if(damage > 0)
	{
		...
		if(gServerInfo.m_ComboPushEffect == 0)
		{
			gObjectManager.CharacterLifeCheck(lpObj,lpTarget,(damage-ShieldDamage),0,flag,effect,((lpSkill==0)?0:lpSkill->m_index),ShieldDamage); //- Default MuEmu
		}

		if(gServerInfo.m_ComboPushEffect >= 1)
		{
			gObjectManager.CharacterLifeCheck(lpObj,lpTarget,(damage-ShieldDamage),0,flag,effect,skill,ShieldDamage); //- Combo Effect When Die
		}
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gServerInfo.m_BloodCastleAllowPk == 0)
	{
	...
	if(gServerInfo.m_RaklionBossEnablePvP == 0)
	{
		if(lpObj->Map == MAP_RAKLION2 && lpTarget->Map == MAP_RAKLION2)
		{
			return 0;
