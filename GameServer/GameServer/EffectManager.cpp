...
void CEffectManager::InsertEffect(LPOBJ lpObj,CEffect* lpEffect) // OK
{
	switch(lpEffect->m_index)
	{
		...
		case EFFECT_MASTER_SEAL_OF_ASCENSION:
			...
			//--
			lpObj->EffectOption.AddExperienceRate -= lpEffect->m_value[0]; //-> Fix Master Scrolls without Master level
			//--
			break;
		...
		case EFFECT_SCROLL_OF_HEALING:
			...
			break;
		//---------------------------------------------------------------------------------
		//-> Start Custom Pentagram
		//---------------------------------------------------------------------------------
		case EFFECT_HAWK_FIGURINE: //-> Brave Hero's Elixir
			gEffectManager.AddEffect(lpObj,0,EFFECT_SWORD_POWER,0,0,0,0,0);
			gEffectManager.AddEffect(lpObj,0,EFFECT_NEIL,0,0,0,0,0);
			lpObj->EffectOption.AddCriticalDamageRate += lpEffect->m_value[0];
			break;	
		//---------------------------------------------------------------------------------
		case EFFECT_GOAT_FIGURINE: //-> Muren's Book of Magic
			gEffectManager.AddEffect(lpObj,0,EFFECT_LIGHTNING_STORM,0,0,0,0,0);
			gEffectManager.AddEffect(lpObj,0,EFFECT_MAGIC_CIRCLE,0,0,0,0,0);
			gEffectManager.AddEffect(lpObj,0,EFFECT_GREATER_MANA,0,0,0,0,0);
			lpObj->EffectOption.AddExcellentDamageRate += lpEffect->m_value[0];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_OAK_CHARM: //-> Kundun's Madness Spell Scroll
			gEffectManager.AddEffect(lpObj,0,EFFECT_BLIND,0,0,0,0,0);
			gEffectManager.AddEffect(lpObj,0,EFFECT_ORDER_OF_SPEED,0,0,0,0,0);
			lpObj->EffectOption.AddPhysiSpeed += lpEffect->m_value[0];
			lpObj->EffectOption.AddMagicSpeed += lpEffect->m_value[0];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_MAPLE_CHARM: //-> Norrwen's Bloodstring Lyra
			gEffectManager.AddEffect(lpObj,0,EFFECT_MAX_AG_BOOST_AURA,0,0,0,0,0);
			gEffectManager.AddEffect(lpObj,0,EFFECT_LESSER_DAMAGE,0,0,0,0,0);
			lpObj->EffectOption.AddMaxMP += lpEffect->m_value[0];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_GOLDEN_OAK_CHARM: //-> Lorencia Knights' Iron Shield
			gEffectManager.AddEffect(lpObj,0,EFFECT_MAX_SD_BOOST_AURA,0,0,0,0,0);
			gEffectManager.AddEffect(lpObj,0,EFFECT_DAMAGE_REFLECT,0,0,0,0,0);
			lpObj->EffectOption.AddMaxHP += lpEffect->m_value[0];
			lpObj->EffectOption.AddMaxSD += lpEffect->m_value[1];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_GOLDEN_MAPLE_CHARM: //-> Thoughtful Sage's Jewel
			gEffectManager.AddEffect(lpObj,0,EFFECT_RED_STORM,0,0,0,0,0);
			gEffectManager.AddEffect(lpObj,0,EFFECT_FIRE_SLASH,0,0,0,0,0);
			gEffectManager.AddEffect(lpObj,0,EFFECT_GREATER_LIFE,0,0,0,0,0);
			lpObj->EffectOption.AddMaxMP += lpEffect->m_value[0];
			lpObj->EffectOption.AddMaxBP += lpEffect->m_value[1];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_WORN_HORSESHOE: //-> Merciless Gladiator's Dagger
			gEffectManager.AddEffect(lpObj,0,EFFECT_SWORD_SLASH,0,0,0,0,0);
			gEffectManager.AddEffect(lpObj,0,EFFECT_SAHAMUTT,0,0,0,0,0);
			break;
		//---------------------------------------------------------------------------------
		//-> End Custom Pentagram
		//---------------------------------------------------------------------------------
...
void CEffectManager::RemoveEffect(LPOBJ lpObj,CEffect* lpEffect) // OK
{
	switch(lpEffect->m_index)
	{
		...
		//---------------------------------------------------------------------------------
		case EFFECT_HAWK_FIGURINE: //-> Brave Hero's Elixir
			gEffectManager.DelEffect(lpObj,EFFECT_SWORD_POWER);
			gEffectManager.DelEffect(lpObj,EFFECT_NEIL);
			lpObj->EffectOption.AddCriticalDamageRate -= lpEffect->m_value[0];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_GOAT_FIGURINE: //-> Muren's Book of Magic
			gEffectManager.DelEffect(lpObj,EFFECT_MAGIC_CIRCLE);
			gEffectManager.DelEffect(lpObj,EFFECT_GREATER_MANA);
			lpObj->EffectOption.AddExcellentDamageRate -= lpEffect->m_value[0];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_OAK_CHARM: //-> Kundun's Madness Spell Scroll
			gEffectManager.DelEffect(lpObj,EFFECT_BLIND);
			gEffectManager.DelEffect(lpObj,EFFECT_ORDER_OF_SPEED);
			lpObj->EffectOption.AddPhysiSpeed -= lpEffect->m_value[0];
			lpObj->EffectOption.AddMagicSpeed -= lpEffect->m_value[0];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_MAPLE_CHARM: //> Norrwen's Bloodstring Lyra
			gEffectManager.DelEffect(lpObj,EFFECT_MAX_AG_BOOST_AURA);
			gEffectManager.DelEffect(lpObj,EFFECT_LESSER_DAMAGE);
			lpObj->EffectOption.AddMaxMP -= lpEffect->m_value[0];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_GOLDEN_OAK_CHARM: //-> Lorencia Knights' Iron Shield
			gEffectManager.DelEffect(lpObj,EFFECT_MAX_SD_BOOST_AURA);
			gEffectManager.DelEffect(lpObj,EFFECT_DAMAGE_REFLECT);
			lpObj->EffectOption.AddMaxHP -= lpEffect->m_value[0];
			lpObj->EffectOption.AddMaxSD -= lpEffect->m_value[1];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_GOLDEN_MAPLE_CHARM: //-> Thoughtful Sage's Jewel
			gEffectManager.DelEffect(lpObj,EFFECT_FIRE_SLASH);
			gEffectManager.DelEffect(lpObj,EFFECT_GREATER_LIFE);
			lpObj->EffectOption.AddMaxMP -= lpEffect->m_value[0];
			lpObj->EffectOption.AddMaxBP -= lpEffect->m_value[1];
			break;
		//---------------------------------------------------------------------------------
		case EFFECT_WORN_HORSESHOE: //-> Merciless Gladiator's Dagger
			gEffectManager.DelEffect(lpObj,EFFECT_SAHAMUTT);
			break;
		//---------------------------------------------------------------------------------
		//-> End Custom Pentagram
		//---------------------------------------------------------------------------------
