...
void CCustomAttack::ReadCustomAttackInfo(char* section,char* path) // OK
{
	...
	//GetPrivateProfileString(section,"CustomAttackText7","",this->m_CustomAttackText7,sizeof(this->m_CustomAttackText7),path);

	//GetPrivateProfileString(section,"CustomAttackText8","",this->m_CustomAttackText8,sizeof(this->m_CustomAttackText8),path);

	//GetPrivateProfileString(section,"CustomAttackText9","",this->m_CustomAttackText9,sizeof(this->m_CustomAttackText9),path);
...
bool CCustomAttack::CommandCustomAttack(LPOBJ lpObj,char* arg) // OK
{
	...
	if (gMapManager.GetMapDisableCustomAttack(lpObj->Map) == 0) //- Custom attack by map
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(791));
		return 0;
	}
