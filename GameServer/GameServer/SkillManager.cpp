...
void CSkillManager::UseDurationSkillAttack(int aIndex,int bIndex,CSkill* lpSkill,BYTE x,BYTE y,BYTE dir,BYTE angle) // OK
{
	...
	if(gServerInfo.m_ComboWithOneSkillEnable == 1)
	{
		...
		if(gServerInfo.m_ComboWithOneSkillEnable == 1)
		{
			...
			if( random < gServerInfo.m_ComboWithOneSkillPercent)
			{
				checkCombo = true;
			}
		} 
...
bool CSkillManager::SkillChangeUse(int aIndex) // OK
{
	...
	for(int n=0;n < 2;n++)
	{
		...
		{
			//change = 625; -> Original s8
			change = 885;
		}
		else if(Ring->m_Index == GET_ITEM(13,164)) // Mini Robot Ring
		{
			//change = 626; -> Original s8
			change = 886;
		}
		else if(Ring->m_Index == GET_ITEM(13,165)) // Great Heavenly Mage Ring
		{
			//change = 642; -> Monkey Original s8
			change = 884;

		}
		else if(Ring->m_Index == GET_ITEM(13,166)) // Brown Panda Ring
		{
			//change = 616; //-> Original s8
			change = 503;
		}
	}
...
void CSkillManager::CGSkillAttackRecv(PMSG_SKILL_ATTACK_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gServerInfo.m_BloodCastleAllowSafeZoneBuff == 0)
	{
	...
	if (gServerInfo.m_BloodCastleAllowSafeZoneBuff == 1)
	{
