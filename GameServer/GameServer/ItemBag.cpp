		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_DropInventory = lpMemScript->GetAsNumber();
				}
				else if(section == 1)
				{
					if(strcmp("end",lpMemScript->GetAsString()) == 0)
...
bool CItemBag::DropItem(LPOBJ lpObj,int map,int x,int y) // OK
{
	...
	for(int n=0;n < this->m_ItemDropCount;n++)
	{
		...
		{
			if(this->m_DropInventory == 1)
				{
					if (gItemManager.CheckItemInventorySpace(lpObj,item.m_Index) == 0)
					{
					...
					{
						GDCreateItemSend(lpObj->Index,0xEB,0,0,item.m_Index,(BYTE)item.m_Level,0,item.m_Option1,item.m_Option2,item.m_Option3,lpObj->Index,item.m_NewOption,item.m_SetOption,item.m_JewelOfHarmonyOption,item.m_ItemOptionEx,item.m_SocketOption,item.m_SocketOptionBonus,0);
						continue;
					}
				}
			else
