...
void CProtect::StartAuth(eAuthServerType ServerType) // OK
{
	...
	if(this->ConnectToAuthServer(this->GetEncryptedString(gProtectString3,sizeof(gProtectString3)),12556) == 0)
	{
...
inline void CProtect::ErrorMessageBox(int Erro) // OK
{
	#if(PROTECT_STATE==1)
	...
	wsprintf(buff,"Não autorizado (#%d).\n\nVerifique sua licença com nosso suporte!",Erro);
