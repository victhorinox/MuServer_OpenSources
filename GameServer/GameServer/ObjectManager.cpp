...
void CObjectManager::CharacterGameCloseSet(int aIndex,int type) // OK
{
	...
	if(lpObj->Connected == OBJECT_ONLINE)
	{
		...
		if((GetTickCount()-lpObj->MySelfDefenseTime) < gServerInfo.m_SelfDefenseTime/2)
		{
...
//	lpObj->CloseCount = 1; //Tempo Relogar
	lpObj->CloseCount = gServerInfo.m_CharacterCloseCount; //Tempo Relogar

//	lpObj->CloseType = type;
	lpObj->CloseType = ((type>2)?0:type); //Godmode hack fix
...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj) // OK
{
	...
	else if(lpObj->Map == MAP_NEW4)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	else if(lpObj->Map == MAP_NEW5)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	...
	else if(lpObj->Map == MAP_ILLUSION_TEMPLE_FINAL1)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	else if(lpObj->Map == MAP_ILLUSION_TEMPLE_FINAL2)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	//--
	...
	else if(lpObj->Map == MAP_NIXIES)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	else if(lpObj->Map == DEEP_DUNGEON)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	...
	else if(lpObj->Map == KUBERA_MINES)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	else if(lpObj->Map == ATLANS_ABYSS)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	else if(lpObj->Map == SCORCHED_CANYON)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	else if(lpObj->Map == GRAY_AIDA)
	{
		result = gGate.GetGate(gMapManager.GetMapDeathGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	//--
...
bool CObjectManager::CharacterUseCustomJewel(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	CUSTOM_JEWEL_UPGRADE_INFO* lpInfo = gCustomJewel.GetUpgradeInfo(lpObj->Inventory[SourceSlot].m_Index);

	if(lpItem->m_Index == lpInfo->ItemIndex)
	{
		lpItem->Convert(lpInfo->CreateItemIndex,lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
	...
	//Divine Sword
	if(lpItem->m_Index == GET_ITEM(0,19) && lpInfo->ItemIndex)
	{
		lpItem->Convert(GET_ITEM(0,51),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}

	//Divine Claw
	if(lpItem->m_Index == GET_ITEM(0,60) && lpInfo->ItemIndex)
	{
		lpItem->Convert(GET_ITEM(0,61),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}

	//Divine Scepter
	if(lpItem->m_Index == GET_ITEM(2,13) && lpInfo->ItemIndex)
	{
		lpItem->Convert(GET_ITEM(2,25),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}

	//Divine Crossbow
	if(lpItem->m_Index == GET_ITEM(4,18) && lpInfo->ItemIndex)
	{
		lpItem->Convert(GET_ITEM(4,30),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}

	//Divine Staff
	if(lpItem->m_Index == GET_ITEM(5,10) && lpInfo->ItemIndex)
	{
		lpItem->Convert(GET_ITEM(5,49),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}

	//Divine Stick
	if(lpItem->m_Index == GET_ITEM(5,36) && lpInfo->ItemIndex)
	{
		lpItem->Convert(GET_ITEM(5,50),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}
	...
	//Custom Pet Luki
	if(lpItem->m_Index == GET_ITEM(13,212) && lpInfo->ItemIndex)
	{
		lpItem->Convert(GET_ITEM(13,213),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}

	//Custom Pet Tony
	if(lpItem->m_Index == GET_ITEM(13,214) && lpInfo->ItemIndex)
	{

		lpItem->Convert(GET_ITEM(13,215),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}

	//Custom Pet Nymph
	if(lpItem->m_Index == GET_ITEM(13,216) && lpInfo->ItemIndex)
	{

		lpItem->Convert(GET_ITEM(13,217),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}

	//Custom Pet Safi
	if(lpItem->m_Index == GET_ITEM(13,218) && lpInfo->ItemIndex)
	{

		lpItem->Convert(GET_ITEM(13,219),lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus);
		this->CharacterMakePreviewCharSet(lpObj->Index);
	}
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		{
			gMonsterReward.AddMonsterBonus(lpTarget,&gObj[SummonIndex]); //- Custom Monster Reward System
