...
void CTrade::ResetTrade(int aIndex) // OK
{
	...
	if(lpObj->Interface.use == 0 || lpObj->Interface.use != INTERFACE_TRADE || lpObj->Interface.state == 0)
	{
		return;
	}
...
void CTrade::CGTradeResponseRecv(PMSG_TRADE_RESPONSE_RECV* lpMsg,int aIndex) // OK
{
	...
	//-> Fix commands to prevent dupes (Trainer)
	...
	//-> Fix commands to prevent dupes (Trainer)
	if(lpObj->Interface.use == INTERFACE_TRAINER)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(64));
		return;
	}
...
void CTrade::CGTradeOkButtonRecv(PMSG_TRADE_OK_BUTTON_RECV* lpMsg,int aIndex) // OK
{
	...
	//Fix Trade delay
	if((GetTickCount()-lpObj->TradeOkTime) < 3000) //6000
	{
		return;
	}
