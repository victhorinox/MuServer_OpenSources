...
void CCashShop::MainProc() // OK
{
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		...
		//WC
		if(gServerInfo.m_CashShopWCoinCDelay >= 1)
		{
			if((GetTickCount()-lpObj->CashShopWCoinCTime) >= ((DWORD)gServerInfo.m_CashShopWCoinCDelay*60000))
			{
				lpObj->CashShopWCoinCTime = GetTickCount();
				this->GDCashShopAddPointSaveSend(lpObj->Index,0,gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_GLOBIN_POINT,gServerInfo.m_CashShopWCoinCValue[lpObj->AccountLevel],-1,-1,-1,-1),gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_GLOBIN_POINT,0,-1,-1,-1,-1),gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_GLOBIN_POINT,0,-1,-1,-1,-1));
		...
		//WP
		if(gServerInfo.m_CashShopWCoinPDelay >= 1)
		{
			if((GetTickCount()-lpObj->CashShopWCoinPTime) >= ((DWORD)gServerInfo.m_CashShopWCoinPDelay*60000))
			{
				lpObj->CashShopWCoinPTime = GetTickCount();
				this->GDCashShopAddPointSaveSend(lpObj->Index,0,gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_GLOBIN_POINT,0,-1,-1,-1,-1),gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_GLOBIN_POINT,gServerInfo.m_CashShopWCoinPValue[lpObj->AccountLevel],-1,-1,-1,-1),gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_GLOBIN_POINT,0,-1,-1,-1,-1));
				this->CGCashShopPointRecv(lpObj->Index);
		...
		//GP
		if(gServerInfo.m_CashShopGoblinPointDelay >= 1)
		{
			if((GetTickCount()-lpObj->CashShopGoblinPointTime) >= ((DWORD)gServerInfo.m_CashShopGoblinPointDelay*60000))
			{
				...
				this->GDCashShopAddPointSaveSend(lpObj->Index,0,gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_GLOBIN_POINT,0,-1,-1,-1,-1),gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_GLOBIN_POINT,0,-1,-1,-1,-1),gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_GLOBIN_POINT,gServerInfo.m_CashShopGoblinPointValue[lpObj->AccountLevel],-1,-1,-1,-1));
				this->CGCashShopPointRecv(lpObj->Index);
...
void CCashShop::CGCashShopOpenRecv(PMSG_CASH_SHOP_OPEN_RECV* lpMsg,int aIndex) // OK
{
	...
	//-> [AntiDupe] Disabled CashShop open when have a item in the Chaos machine
