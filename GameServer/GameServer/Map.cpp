...
bool CMap::MoneyItemDrop(int money,int x,int y) // OK
{
	...
	for(int n=0;n < MAX_MAP_ITEM;n++)
	{
		if(this->m_Item[count].IsItem() == 0)
		{
			...
			this->m_Item[count].m_PickUpTime = 0;
...
bool CMap::CheckItemGive(int aIndex,int index) // OK
{
	...
	if(OBJECT_RANGE(this->m_Item[index].m_UserIndex) != 0 && this->m_Item[index].m_PickUpTime > GetTickCount() && this->m_Item[index].m_UserIndex != aIndex)
	{
		loot = 0;
	...
	if(loot == 0)
	{
		if(gServerInfo.m_ItemDropPickUpTime >= 1)
		{
