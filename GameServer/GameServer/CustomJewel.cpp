...
bool CCustomJewel::CheckCustomJewelApplyItem(int ItemIndex,CItem* lpItem) // OK
{
	...
	if(lpItem->m_Index > GET_ITEM(12,6) && (lpItem->m_Index < GET_ITEM(12,36) || lpItem->m_Index > GET_ITEM(12,43)) && lpItem->m_Index != GET_ITEM(12,49) && lpItem->m_Index != GET_ITEM(12,50) && (lpItem->m_Index < GET_ITEM(12,200) || lpItem->m_Index > GET_ITEM(12,265)) && lpItem->m_Index != GET_ITEM(13,30) && (lpItem->m_Index <= GET_ITEM(13,200) && lpItem->m_Index >= GET_ITEM(13,210)) && (lpItem->m_Index <= GET_ITEM(13,212) && lpItem->m_Index >= GET_ITEM(13,511))  && gCustomWing.CheckCustomWingByItem(lpItem->m_Index) == 0)
	{
		return 0;
	}
	...
	if((GetLargeRand()%100) < gCustomJewel.GetCustomJewelSuccessRate(lpObj->Inventory[SourceSlot].m_Index,lpObj->AccountLevel))
	{
		...
		for(std::vector<CUSTOM_JEWEL_UPGRADE_INFO>::iterator it=this->m_UpgradeInfo.begin();it != this->m_UpgradeInfo.end();it++)
		{
			if(it->Index == lpInfo->Index)
			{
				...
				{	
					if(it->Type == 1) // Opções originais do item
					{
					...
					else // Sem nenhuma opção
					{
						lpItem->Convert(it->ItemNew,0,0,0,0,0,0,0,0,0);
					}
