...
void CCustomBuyVip::BuyVip(int aIndex,BUYPREMIUM_REQ* lpMsg)
{
	...
	//Fix Custom Store + Buy Vip
	if(lpObj->Interface.use == INTERFACE_PERSONAL_SHOP || lpObj->Interface.type == INTERFACE_TRADE || lpObj->Interface.type == INTERFACE_SHOP || lpObj->Interface.type == INTERFACE_WAREHOUSE)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(536));
		return;
	}
