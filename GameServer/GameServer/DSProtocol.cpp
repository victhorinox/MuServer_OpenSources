...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	if( lpObj->Authority == 32)
	{
		...
		{
			PostMessageGameMasterON(lpObj->Name,gMessage.GetMessage(639));
		}

		if (gServerInfo.m_OnlineGmSwitch == 2) 
		{
			gNotice.GCNoticeSendToAll(1,0,0,0,0,0,gMessage.GetMessage(639),lpObj->Name);
		}

		if (gServerInfo.m_OnlineGmSwitch == 3) 
		{
	...
	else
	{
		...
		if (gServerInfo.m_OnlineUserSwitch == 2) 
		{
			gNotice.GCNoticeSendToAll(1,0,0,0,0,0,gMessage.GetMessage(640),lpObj->Name);
		}

		if (gServerInfo.m_OnlineUserSwitch == 3) 
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(640),lpObj->Name);
		}
	}
