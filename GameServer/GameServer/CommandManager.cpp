...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message, int Npc) // OK
{
	...
	//-> Fix commands to prevent dupes (Warehouse)
	...
	//-> Fix commands to prevent dupes (Party)
	...
	//-> Fix commands to prevent dupes (ChaosGoblin)
	...
	//-> Fix commands to prevent dupes (Store)
	...
	//-> Fix commands to prevent dupes (CashShop)
	...
	//-> Fix commands to prevent dupes (Warehouse)
	if(lpObj->Interface.use == INTERFACE_WAREHOUSE)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(64));
		return 0;
	}
	...
	//-> Fix commands to prevent dupes (Party)
	if(lpObj->Interface.use == INTERFACE_PARTY)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(64));
		return 0;
	}
	...
	//-> Fix commands to prevent dupes (ChaosGoblin)
	if(lpObj->Interface.use == INTERFACE_CHAOS_BOX)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(64));
		return 0;
	}
	...
	//-> Fix commands to prevent dupes (Store)
	if(lpObj->Interface.use == INTERFACE_PERSONAL_SHOP)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(64));
		return 0;
	}
	...
	//-> Fix commands to prevent dupes (CashShop)
	if(lpObj->Interface.use == INTERFACE_CASH_SHOP)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(64));
		return 0;
	}
	...
	//-> Try Fix New Dupe
	if(lpObj->Map == 3 && lpObj->X >= 177 && lpObj->Y >= 100 && lpObj->TX <= 183 && lpObj->TY <= 106)
	{
	...
	switch(code)
	{
		...
		case COMMAND_SET_PK:
			...
			break;
		case COMMAND_BLOCK_CHAT:
			Result = CommandBlockChat(lpObj,argument,Npc);
			break;
		case COMMAND_ONLINE:
			this->CommandOnline(lpObj,argument);
			break;
		case COMMAND_TIME:
			this->CommandTime(lpObj,argument);
			break;
		case COMMAND_SET_LEVEL:
			this->CommandSetLevel(lpObj,argument);
			break;
		case COMMAND_SET_POINT:
			this->CommandSetLevelUpPoint(lpObj,argument);
			break;
		case COMMAND_SET_STR:
			this->CommandSetStrength(lpObj,argument);
			break;
		case COMMAND_SET_AGI:
			this->CommandSetAgility(lpObj,argument);
			break;
		case COMMAND_SET_VIT:
			this->CommandSetVitality(lpObj,argument);
			break;
		case COMMAND_SET_ENE:
			this->CommandSetEnergy(lpObj,argument);
			break;
		case COMMAND_SET_CMD:
			this->CommandSetCommand(lpObj,argument);
			break;
		case COMMAND_SET_RESET:
			this->CommandSetResetCount(lpObj,argument);
			break;
		case COMMAND_SET_MRESET:
			this->CommandSetMasterResetCount(lpObj,argument);
			break;

		case COMMAND_SEND1:
			gBankEx.SendZenCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND2:
			gBankEx.SendBlessCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND3:
			gBankEx.SendSoulCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND4:
			gBankEx.SendChaosCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND5:
			gBankEx.SendLifeCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND6:
			gBankEx.SendCreationCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV1:
			gBankEx.RecvZenCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV2:
			gBankEx.RecvBlessCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV3:
			gBankEx.RecvSoulCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV4:
			gBankEx.RecvChaosCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV5:
			gBankEx.RecvLifeCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV6:
			gBankEx.RecvCreationCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV7:
			gBankEx.RecvHarmonyCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND7:
			gBankEx.SendHarmonyCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV8:
			gBankEx.RecvGuardianCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND8:
			gBankEx.SendGuardianCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV9:
			gBankEx.RecvGemstoneCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND9:
			gBankEx.SendGemstoneCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV10:
			gBankEx.RecvLowerRefinCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND10:
			gBankEx.SendLowerRefinCustomVault(lpObj,argument);
			break;
		case COMMAND_RECV11:
			gBankEx.RecvHighRefinCustomVault(lpObj,argument);
			break;
		case COMMAND_SEND11:
			gBankEx.SendHighRefinCustomVault(lpObj,argument);
			break;
		case COMMAND_DROPALL:
            Result = CommandDropAll(lpObj,argument);
            break;
		case COMMAND_DROPALL_MAP:
            Result = CommandDropAllMap(lpObj,argument);
            break;
		case COMMAND_MAKEALL:
            Result = CommandMakeAll(lpObj,argument);
            break;
		case COMMAND_MAKEALL_MAP:
            Result = CommandMakeAllMap(lpObj,argument);
            break;
		case COMMAND_DROP_FOR:
            Result = CommandDropFor(lpObj,argument);
            break;
		case COMMAND_MAKE_FOR:
            Result = CommandMakeFor(lpObj,argument);
            break;
		case COMMAND_MAKESET_FOR:
            Result = CommandMakeSetFor(lpObj,argument);
            break;
...
void CCommandManager::CommandResetAutoProc(LPOBJ lpObj) // OK
{
	...
	{
		lpObj->AutoResetEnable = 0;
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(534));
		return;
	}
...
void CCommandManager::DGCommandResetRecv(SDHP_COMMAND_RESET_RECV* lpMsg) // OK
{
	...
	if(gResetTable.GetResetMoney(lpObj) >= 1) 
	{
		lpObj->Money -= gResetTable.GetResetMoney(lpObj);
	}
...
void CCommandManager::DGCommandMasterResetRecv(SDHP_COMMAND_MASTER_RESET_RECV* lpMsg) // OK
{
	...
	if (lpObj->Strength <= gServerInfo.m_CommandMasterResetRequiredStr)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You Lacking Points");
		return;
	}

	if (lpObj->Dexterity <= gServerInfo.m_CommandMasterResetRequiredAgi)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You Lacking Points");
		return;
	}

	if (lpObj->Vitality <= gServerInfo.m_CommandMasterResetRequiredVit)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You Lacking Points");
		return;
	}

	if (lpObj->Energy <= gServerInfo.m_CommandMasterResetRequiredEne)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You Lacking Points");
		return;
	}
	...
	{
		if (lpObj->Leadership <= gServerInfo.m_CommandMasterResetRequiredCom)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You Lacking Points");
			return;
		}
...
bool CCommandManager::CommandOpenWarehouse(LPOBJ lpObj) // OK
{
	...
	if (gItemManager.IsChaosBoxHasItem(lpObj) || gItemManager.IsTradeHasItem(lpObj))
	{
		return 0;
	}
	//--
...
bool CCommandManager::CommandReAdd(LPOBJ lpObj,char* arg,int Npc)
{
	...
	if(lpObj->Class == CLASS_DL)
	{
		...
		{
			lpObj->LevelUpPoint += (lpObj->Strength+lpObj->Dexterity+lpObj->Vitality+lpObj->Energy);
...
bool CCommandManager::CommandHelper(LPOBJ lpObj,char* arg,int Npc)
{
	...
	//Command Ajuda by map
	if (gMapManager.GetMapCommandHelp(lpObj->Map) == 0)
	{
...
bool CCommandManager::CommandDisablePvp(LPOBJ lpObj,char* arg,int Npc)
{
	...
	if (gMapManager.GetMapCommandOffPvP(lpObj->Map) == 0)
	{
...
bool CCommandManager::CommandBlockChat(LPOBJ lpObj,char* arg, int Npc)
{
	char name[11] = {0};
	...
	//int BlockTime = this->GetNumber(arg,1);
	int BlockTime = this->GetNumber(arg,0);
	...
	lpTarget->ChatLimitTime += BlockTime;
	...
	gLog.Output(LOG_COMMAND,"[CommandBlockChat][%s][%s] - (Time: %d)",lpTarget->Account,lpTarget->Name,lpTarget->ChatLimitTime);

	return 1;
}


bool CCommandManager::CommandOnline(LPOBJ lpObj,char* arg)
{
	gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(492),gObjTotalUser,gServerInfo.m_ServerMaxUserNumber);
	
	gLog.Output(LOG_COMMAND,"[CommandOnline][%s][%s]",lpObj->Account,lpObj->Name);

	return 1;
}

bool CCommandManager::CommandTime(LPOBJ lpObj,char* arg)
{
	SYSTEMTIME time;
	
	GetLocalTime(&time);
	
	gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(493),time.wHour,time.wMinute,time.wSecond);
	
	gLog.Output(LOG_COMMAND,"[CommandTime][%s][%s]",lpObj->Account,lpObj->Name);
...
bool CCommandManager::CommandSetLevel(LPOBJ lpObj,char* arg) // OK
{
	...
	if(level >= 401)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(495));
		return 0;
	}
...
bool CCommandManager::CommandSetLevelUpPoint(LPOBJ lpObj,char* arg) // OK
{
	char name[11] = {0};
	...
	int point = this->GetNumber(arg,1);
	...
	lpTarget->LevelUpPoint = point;

	GCLevelUpSend(lpTarget);

	gLog.Output(LOG_COMMAND,"[CommandSetLevelUpPoint][%s][%s] - (Name: %s, LevelUpPoint: %d)",lpObj->Account,lpObj->Name,name,point);

	return 1;
}

bool CCommandManager::CommandSetStrength(LPOBJ lpObj,char* arg) // OK
{
	char name[11] = {0};
	...
	gLog.Output(LOG_COMMAND,"[CommandSetStrength][%s][%s] - (Name: %s, Strength: %d)",lpObj->Account,lpObj->Name,name,str);

	return 1;
}

bool CCommandManager::CommandSetAgility(LPOBJ lpObj,char* arg) // OK
{
	char name[11] = {0};
	...
	lpTarget->Dexterity = agi;

	GCLevelUpSend(lpTarget);

	GCNewCharacterInfoSend(lpTarget);

	gLog.Output(LOG_COMMAND,"[CommandSetAgility][%s][%s] - (Name: %s, Agility: %d)",lpObj->Account,lpObj->Name,name,agi);

	return 1;
}

bool CCommandManager::CommandSetVitality(LPOBJ lpObj,char* arg) // OK
{
	char name[11] = {0};
	...
	gLog.Output(LOG_COMMAND,"[CommandSetVitality][%s][%s] - (Name: %s, Vitality: %d)",lpObj->Account,lpObj->Name,name,vit);

	return 1;
}

bool CCommandManager::CommandSetEnergy(LPOBJ lpObj,char* arg) // OK
{
	char name[11] = {0};
	...
	gLog.Output(LOG_COMMAND,"[CommandSetEnergy][%s][%s] - (Name: %s, Energy: %d)",lpObj->Account,lpObj->Name,name,ene);

	return 1;
}

bool CCommandManager::CommandSetCommand(LPOBJ lpObj,char* arg) // OK
{
	char name[11] = {0};
	...
	{
		lpTarget->Leadership = cmd;
		
		GCLevelUpSend(lpTarget);
		
		GCNewCharacterInfoSend(lpTarget);
		
		gLog.Output(LOG_COMMAND,"[CommandSetCommand][%s][%s] - (Name: %s, Command: %d)",lpObj->Account,lpObj->Name,name,cmd);
	}

	else
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(496));
		return 0;
	}
...
bool CCommandManager::CommandSetResetCount(LPOBJ lpObj,char* arg) // OK
{
	char name[11] = {0};
	...
	if(reset >= gServerInfo.m_CommandResetLimit[lpObj->AccountLevel])
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(495));
		return 0;
	}
	...
	lpTarget->Reset = reset-1;
		
	gLog.Output(LOG_COMMAND,"[CommandSetResetCount][%s][%s] - (Name: %s, Reset: %d)",lpObj->Account,lpObj->Name,name,reset);

	return 1;
}

bool CCommandManager::CommandSetMasterResetCount(LPOBJ lpObj,char* arg) // OK
{
	char name[11] = {0};
	...
	int mreset = this->GetNumber(arg,1);

	if(mreset >= gServerInfo.m_CommandMasterResetLimit[lpObj->AccountLevel])
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(495));
		return 0;
	}
	...
	lpTarget->MasterReset = mreset-1;
		
	gLog.Output(LOG_COMMAND,"[CommandSetMasterResetCount][%s][%s] - (Name: %s, MasterReset: %d)",lpObj->Account,lpObj->Name,name,mreset);
...
bool CCommandManager::CommandDropAll(LPOBJ lpObj,char* arg) // OK
{
	...
	gLog.Output(LOG_COMMAND,"[CommandDropAll][%s][%s] - (NameItem: %s) - (Section: %d, Type: %d, Level: %d, Skill: %d, Luck: %d, Option: %d, Exc: %d, Set: %d, Sockets: %d, [Amount: %d])",lpObj->Account,lpObj->Name,gItemManager.GetItemName(GET_ITEM(section,type)),section,type,level,skill,luck,option,exc,set,socket);
	
	return 1;
}

bool CCommandManager::CommandDropAllMap(LPOBJ lpObj,char* arg) // OK
{
	int qtd=0,map,section,type,level,skill,luck,option,exc,set,socket;
	
	BYTE ItemSocketOption[MAX_SOCKET_OPTION] = {0xFF,0xFF,0xFF,0xFF,0xFF};
	
	map = this->GetNumber(arg,0);
	section = this->GetNumber(arg,1);
	type = this->GetNumber(arg,2);
	level = this->GetNumber(arg,3);
	skill = this->GetNumber(arg,4);
	...
	gLog.Output(LOG_COMMAND,"[CommandDropAllMap][%s][%s] - (Map: %d, NameItem: %s) - (Section: %d, Type: %d, Level: %d, Skill: %d, Luck: %d, Option: %d, Exc: %d, Set: %d, Sockets: %d, [Amount: %d])",lpObj->Account,lpObj->Name,map,gItemManager.GetItemName(GET_ITEM(section,type)),section,type,level,skill,luck,option,exc,set,socket,(qtd>0)?qtd:1);
	
	return 1;
}

bool CCommandManager::CommandMakeAll(LPOBJ lpObj,char* arg) // OK
{
	int section = this->GetNumber(arg,0);
	int type = this->GetNumber(arg,1);
	int level = this->GetNumber(arg,2);
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObjIsConnectedGP(n) != 0 && gObj[n].Index != lpObj->Index)
		{
			if(gObj[n].Connected > OBJECT_LOGGED)
			{
				gNotice.GCNoticeSend(gObj[n].Index,1,0,0,0,0,0,"Item has been make Gift by GM");

				GDCreateItemSend(gObj[n].Index,0xEB,0,0,GET_ITEM(section,type),level,0,skill,luck,option,-1,exc,set,0,0,ItemSocketOption,0xFF,0);
				//GDCreateItemSend(gObj[n].Index,gObj[n].Map,(BYTE)gObj[n].X,(BYTE)gObj[n].Y,GET_ITEM(section,type),level,0,skill,luck,option,-1,exc,set,0,0,ItemSocketOption,0xFF,0);
			}
		}
	}

	gLog.Output(LOG_COMMAND,"[CommandMakeAll][%s][%s] - (NameItem: %s) - (Section: %d, Type: %d, Level: %d, Skill: %d, Luck: %d, Option: %d, Exc: %d, Set: %d, Sockets: %d, [Amount: %d])",lpObj->Account,lpObj->Name,gItemManager.GetItemName(GET_ITEM(section,type)),section,type,level,skill,luck,option,exc,set,socket);
	
	return 1;
}

bool CCommandManager::CommandMakeAllMap(LPOBJ lpObj,char* arg) // OK
{
	int qtd=0,map,section,type,level,skill,luck,option,exc,set,socket;
	...
	gLog.Output(LOG_COMMAND,"[CommandMakeAllMap][%s][%s] - (Map: %d, NameItem: %s) - (Section: %d, Type: %d, Level: %d, Skill: %d, Luck: %d, Option: %d, Exc: %d, Set: %d, Sockets: %d, [Amount: %d])",lpObj->Account,lpObj->Name,map,gItemManager.GetItemName(GET_ITEM(section,type)),section,type,level,skill,luck,option,exc,set,socket,(qtd>0)?qtd:1);
	
	return 1;
}

bool CCommandManager::CommandDropFor(LPOBJ lpObj,char* arg) // OK
{
	...
	if(lpTarget == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Usuario nao existe");
		return 0;
	}
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObjIsConnectedGP(n) != 0 && gObj[n].Index == lpTarget->Index)
		{
			if(gObj[n].Connected > OBJECT_LOGGED)
			{
				gNotice.GCNoticeSend(gObj[n].Index,1,0,0,0,0,0,"Item has been dropped Gift by GM");
	...
	gLog.Output(LOG_COMMAND,"[CommandDropFor][%s][%s] - (NameItem: %s) - (Section: %d, Type: %d, Level: %d, Skill: %d, Luck: %d, Option: %d, Exc: %d, Set: %d, Sockets: %d, [Amount: %d])",lpObj->Account,lpObj->Name,gItemManager.GetItemName(GET_ITEM(section,type)),section,type,level,skill,luck,option,exc,set,socket);
	
	return 1;
}

bool CCommandManager::CommandMakeFor(LPOBJ lpObj,char* arg) // OK
{
	int section = this->GetNumber(arg,1);
	int type = this->GetNumber(arg,2);
	int level = this->GetNumber(arg,3);
	...
	GDCreateItemSend(lpTarget->Index,0xEB,0,0,GET_ITEM(section,type),level,0,skill,luck,option,-1,exc,set,0,0,ItemSocketOption,0xFF,0);
		
	gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You used a Make Command for a Player");

	gLog.Output(LOG_COMMAND,"[CommandMakeFor][%s][%s] - (Type: %d, Level: %d, Skill: %d, Luck: %d, Option: %d, Exc: %d, Set: %d, Sockets: %d)",lpObj->Account,lpObj->Name,type,level,skill,luck,option,exc,set, socket);
		
	return 1;
}

bool CCommandManager::CommandMakeSetFor(LPOBJ lpObj,char* arg) // OK
{
	int section = this->GetNumber(arg,0);
	int type = this->GetNumber(arg,1);
	int level = this->GetNumber(arg,2);
	...
	gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You used a MakeSet Command for a Player");

	gLog.Output(LOG_COMMAND,"[CommandMakeSetFor][%s][%s] - (Type: %d, Level: %d, Skill: %d, Luck: %d, Option: %d, Exc: %d, Set: %d, Sockets: %d)",lpObj->Account,lpObj->Name,type,level,skill,luck,option,exc,set, socket);
