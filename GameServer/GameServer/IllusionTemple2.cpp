// IllusionTemple2.cpp: implementation of the CIllusionTemple class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "IllusionTemple2.h"
...
CIllusionTemple2 gIllusionTemple2;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CIllusionTemple2::CIllusionTemple2() // OK
{
	for(int n=0;n < MAX_IT_LEVEL;n++)
	{
		ILLUSION_TEMPLE_LEVEL* lpLevel = &this->m_IllusionTemple2Level[n];
	...
	memset(this->m_IllusionTemple2ExperienceTable,0,sizeof(this->m_IllusionTemple2ExperienceTable));
}
...
void CIllusionTemple2::Init() // OK
{
	for(int n=0;n < MAX_IT_LEVEL;n++)
	{
		if(gServerInfo.m_IllusionTemple2Event == 0)
		{
			this->SetState(&this->m_IllusionTemple2Level[n],IT_STATE_BLANK);
		}
		else
		{
			this->SetState(&this->m_IllusionTemple2Level[n],IT_STATE_EMPTY);
		}
	}
}

void CIllusionTemple2::Load(char* path) // OK
{
	...
	this->m_IllusionTemple2StartTime.clear();
		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 1)
				{
					...
					this->m_IllusionTemple2StartTime.push_back(info);
				}
				else if(section == 2)
				{
					if(strcmp("end",lpMemScript->GetAsString()) == 0)
					...
					this->m_IllusionTemple2ExperienceTable[level][0] = lpMemScript->GetAsNumber();

					this->m_IllusionTemple2ExperienceTable[level][1] = lpMemScript->GetAsNumber();
				}
				else if(section == 3)
				{
					if(strcmp("end",lpMemScript->GetAsString()) == 0)
					...
					this->m_IllusionTemple2Reward[level][0] = lpMemScript->GetAsNumber();

					this->m_IllusionTemple2Reward[level][1] = lpMemScript->GetAsNumber();

					this->m_IllusionTemple2Reward[level][2] = lpMemScript->GetAsNumber();
				}
				else
				{
					break;
...
void CIllusionTemple2::MainProc() // OK
{
	for(int n=0;n < MAX_IT_LEVEL;n++)
	{
		ILLUSION_TEMPLE_LEVEL* lpLevel = &this->m_IllusionTemple2Level[n];
...
void CIllusionTemple2::ProcState_BLANK(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{

}

void CIllusionTemple2::ProcState_EMPTY(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
...
void CIllusionTemple2::ProcState_STAND(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	this->CheckUser(lpLevel);

	if(this->GetUserCount(lpLevel,IT_TEAM_Y) < (gServerInfo.m_IllusionTemple2MinUser/2) || this->GetUserCount(lpLevel,IT_TEAM_B) < (gServerInfo.m_IllusionTemple2MinUser/2))
	{
	...
	this->GCIllusionTemple2BattleInfoSend(lpLevel);
}

void CIllusionTemple2::ProcState_START(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	this->CheckUser(lpLevel);
...
void CIllusionTemple2::ProcState_CLEAN(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	this->CheckUser(lpLevel);
...
void CIllusionTemple2::SetState(ILLUSION_TEMPLE_LEVEL* lpLevel,int state) // OK
{
	lpLevel->State = state;
...
void CIllusionTemple2::SetState_BLANK(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	//LogAdd(LOG_BLACK,"[Illusion Temple] (%d) SetState BLANK",(lpLevel->Level+1));
}

void CIllusionTemple2::SetState_EMPTY(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	lpLevel->EnterEnabled = 0;
	lpLevel->MinutesLeft = -1;
	lpLevel->TimeCount = 0;
	...
	this->GCIllusionTemple2ChangeScenarioSend(lpLevel,0,-1);
...
void CIllusionTemple2::SetState_STAND(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	lpLevel->EnterEnabled = 0;
	lpLevel->MinutesLeft = -1;
	lpLevel->TimeCount = 0;
	...
	this->GCIllusionTemple2ChangeScenarioSend(lpLevel,1,-1);
...
void CIllusionTemple2::SetState_START(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	lpLevel->EnterEnabled = 0;
	lpLevel->MinutesLeft = -1;
	lpLevel->TimeCount = 0;
	...
	this->GCIllusionTemple2ChangeScenarioSend(lpLevel,2,-1);
...
void CIllusionTemple2::SetState_CLEAN(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	lpLevel->EnterEnabled = 0;
	lpLevel->MinutesLeft = -1;
	lpLevel->TimeCount = 0;
	...
	if(lpLevel->Score[IT_TEAM_Y] >= 2 && lpLevel->Score[IT_TEAM_Y] > lpLevel->Score[IT_TEAM_B])
	{
		...
		lpLevel->TeamRewardExperience[IT_TEAM_Y] = (this->m_IllusionTemple2ExperienceTable[lpLevel->Level][0]+((lpLevel->Score[IT_TEAM_Y]-lpLevel->Score[IT_TEAM_B])*this->m_IllusionTemple2ExperienceTable[lpLevel->Level][1]))*2;

		if(lpLevel->Score[IT_TEAM_B] == 0)
		{
			lpLevel->TeamRewardExperience[IT_TEAM_B] = this->m_IllusionTemple2ExperienceTable[lpLevel->Level][0];
		}
		else
		{
			lpLevel->TeamRewardExperience[IT_TEAM_B] = this->m_IllusionTemple2ExperienceTable[lpLevel->Level][0]+(lpLevel->Score[IT_TEAM_B]*this->m_IllusionTemple2ExperienceTable[lpLevel->Level][1]);
		}
	}
	else if(lpLevel->Score[IT_TEAM_B] >= 2 && lpLevel->Score[IT_TEAM_B] > lpLevel->Score[IT_TEAM_Y])
	{
		...
		lpLevel->TeamRewardExperience[IT_TEAM_B] = (this->m_IllusionTemple2ExperienceTable[lpLevel->Level][0]+((lpLevel->Score[IT_TEAM_B]-lpLevel->Score[IT_TEAM_Y])*this->m_IllusionTemple2ExperienceTable[lpLevel->Level][1]))*2;

		if(lpLevel->Score[IT_TEAM_Y] == 0)
		{
			lpLevel->TeamRewardExperience[IT_TEAM_Y] = this->m_IllusionTemple2ExperienceTable[lpLevel->Level][0];
		}
		else
		{
			lpLevel->TeamRewardExperience[IT_TEAM_Y] = this->m_IllusionTemple2ExperienceTable[lpLevel->Level][0]+(lpLevel->Score[IT_TEAM_Y]*this->m_IllusionTemple2ExperienceTable[lpLevel->Level][1]);
		}
	}
	else
	{
	...
	for(int n=0;n < MAX_IT_USER;n++)
	{
		...
		if(lpLevel->User[n].Team == lpLevel->WinnerTeam)
		{
			...
			if (this->m_IllusionTemple2Reward[lpLevel->Level][0] > 0 || this->m_IllusionTemple2Reward[lpLevel->Level][1] > 0 || this->m_IllusionTemple2Reward[lpLevel->Level][2] > 0)
			{
				//Reward
				GDSetCoinSend(lpObj->Index, this->m_IllusionTemple2Reward[lpLevel->Level][0], this->m_IllusionTemple2Reward[lpLevel->Level][1], this->m_IllusionTemple2Reward[lpLevel->Level][2],"IT");
	...
	this->GCIllusionTemple2ChangeScenarioSend(lpLevel,3,-1);

	this->ClearMonster(lpLevel);

#if (GAMESERVER_UPDATE <= 603)
	this->GCIllusionTemple2RewardScoreSend(lpLevel);
#endif
...
void CIllusionTemple2::CheckSync(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	if(this->m_IllusionTemple2StartTime.empty() != 0)
	{
	...
	for(std::vector<ILLUSION_TEMPLE_START_TIME>::iterator it=this->m_IllusionTemple2StartTime.begin();it != this->m_IllusionTemple2StartTime.end();it++)
	{
...
int CIllusionTemple2::GetState(int level) // OK
{
	...
	return this->m_IllusionTemple2Level[level].State;
}

int CIllusionTemple2::GetRemainTime(int level) // OK
{
	if(IT_LEVEL_RANGE(level) == 0)
	{
		return 0;
...
int CIllusionTemple2::GetEnterEnabled(int level) // OK
{
	if(IT_LEVEL_RANGE(level) == 0)
	{
		return 0;
	...
	return this->m_IllusionTemple2Level[level].EnterEnabled;
}

int CIllusionTemple2::GetEnteredUserCount(int level) // OK
{
	if(IT_LEVEL_RANGE(level) == 0)
	{
		return 0;
	...
	return this->GetUserCount(&this->m_IllusionTemple2Level[level],IT_TEAM_NONE);
}

bool CIllusionTemple2::AddUser(ILLUSION_TEMPLE_LEVEL* lpLevel,int aIndex) // OK
{
...
bool CIllusionTemple2::DelUser(ILLUSION_TEMPLE_LEVEL* lpLevel,int aIndex) // OK
{
	if(OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
ILLUSION_TEMPLE_USER* CIllusionTemple2::GetUser(ILLUSION_TEMPLE_LEVEL* lpLevel,int aIndex) // OK
{
	if(OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
void CIllusionTemple2::CleanUser(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_IT_USER;n++)
	{
...
void CIllusionTemple2::ClearUser(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_IT_USER;n++)
	{
		if(OBJECT_RANGE(lpLevel->User[n].Index) == 0)
...
void CIllusionTemple2::CheckUser(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_IT_USER;n++)
	{
		if(OBJECT_RANGE(lpLevel->User[n].Index) == 0)
...
int CIllusionTemple2::GetUserCount(ILLUSION_TEMPLE_LEVEL* lpLevel,int team) // OK
{
	int count = 0;
...
int CIllusionTemple2::GetUserJoinTeam(LPOBJ lpObj) // OK
{
	...
	ILLUSION_TEMPLE_USER* lpUser = this->GetUser(&this->m_IllusionTemple2Level[level],lpObj->Index);
...
int CIllusionTemple2::GetUserAbleLevel(LPOBJ lpObj) // OK
{
	int level = -1;
...
bool CIllusionTemple2::GetUserRespawnLocation(LPOBJ lpObj,int* gate,int* map,int* x,int* y,int* dir,int* level) // OK
{
	int IllusionTemple2Level = GET_IT_LEVEL(lpObj->Map);

	if(IT_LEVEL_RANGE(IllusionTemple2Level) == 0)
	{
		return 0;
	}

	ILLUSION_TEMPLE_USER* lpUser = this->GetUser(&this->m_IllusionTemple2Level[IllusionTemple2Level],lpObj->Index);

	if(lpUser != 0 && lpUser->Team != IT_TEAM_NONE)
	{
		if(gGate.GetGate(((148+(lpUser->Team*6))+IllusionTemple2Level),gate,map,x,y,dir,level) == 0)
		{
			return 0;
		}
	}
...
void CIllusionTemple2::SearchUserDropEventItem(int aIndex) // OK
{
	...
	ILLUSION_TEMPLE_LEVEL* lpLevel = &this->m_IllusionTemple2Level[level];
	...
	this->GCIllusionTemple2EventItemOwnerS8Send(lpLevel, aIndex, 1);
}

void CIllusionTemple2::SearchUserDeleteEventItem(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
void CIllusionTemple2::GiveUserRewardExperience(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_IT_USER;n++)
	{
		if(OBJECT_RANGE(lpLevel->User[n].Index) == 0)
...
bool CIllusionTemple2::AddMonster(ILLUSION_TEMPLE_LEVEL* lpLevel,int aIndex) // OK
{
	if(OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
bool CIllusionTemple2::DelMonster(ILLUSION_TEMPLE_LEVEL* lpLevel,int aIndex) // OK
{
	if(OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
int* CIllusionTemple2::GetMonster(ILLUSION_TEMPLE_LEVEL* lpLevel,int aIndex) // OK
{
	if(OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
void CIllusionTemple2::CleanMonster(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_IT_MONSTER;n++)
	{
...
void CIllusionTemple2::ClearMonster(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_IT_MONSTER;n++)
	{
		if(OBJECT_RANGE(lpLevel->MonsterIndex[n]) != 0)
...
int CIllusionTemple2::GetMonsterCount(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	int count = 0;
...
void CIllusionTemple2::SetStoneStatue(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
#if (GAMESERVER_UPDATE <= 603)
	if(OBJECT_RANGE(lpLevel->StoneStatueIndex) != 0)
	{
	...
	//S8
	//this->GCIllusionTemple2StoneInfoSend(lpLevel,0,0);
}

void CIllusionTemple2::SetMonster(ILLUSION_TEMPLE_LEVEL* lpLevel,int MonsterClass) // OK
{
...
void CIllusionTemple2::NpcStoneStatue(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
...
void CIllusionTemple2::NpcYellowStorage(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	if(gServerInfo.m_IllusionTemple2Event == 0)
	{
		return;
...
void CIllusionTemple2::NpcBlueStorage(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	if(gServerInfo.m_IllusionTemple2Event == 0)
	{
		return;
...
void CIllusionTemple2::NpcMirageTheMummy(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	if(gServerInfo.m_IllusionTemple2Event == 0)
	{
		return;
	...
	pMsg.level[1] = this->GetUserCount(&this->m_IllusionTemple2Level[level],IT_TEAM_NONE);

	DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
}

bool CIllusionTemple2::CheckEventItemSerial(int map,CMapItem* lpItem) // OK
{
	...
	if(this->m_IllusionTemple2Level[level].EventItemSerial == -1)
	{
		return 0;
	}

	if(lpItem->m_Index == GET_ITEM(14,64) && lpItem->m_Serial == this->m_IllusionTemple2Level[level].EventItemSerial)
	{
		return 1;
	}
...
void CIllusionTemple2::SetEventItemSerial(int map,int index,int serial) // OK
{
	int level = GET_IT_LEVEL(map);
	...
	{
		this->m_IllusionTemple2Level[level].EventItemSerial = serial;
	}
}

void CIllusionTemple2::DropEventItem(int map,int number,int aIndex) // OK
{
	int level = GET_IT_LEVEL(map);
	...
	this->m_IllusionTemple2Level[level].EventItemNumber = number;

	this->m_IllusionTemple2Level[level].EventItemOwner = -1;

	this->m_IllusionTemple2Level[level].EventItemLevel = -1;

#if (GAMESERVER_UPDATE <= 603)
	this->NoticeSendToAll(&this->m_IllusionTemple2Level[level],0,gMessage.GetMessage(183),gObj[aIndex].Name);
#endif
}

void CIllusionTemple2::GetEventItem(int map,int aIndex,CMapItem* lpItem) // OK
{
	int level = GET_IT_LEVEL(map);
	...
	this->m_IllusionTemple2Level[level].EventItemNumber = -1;

	this->m_IllusionTemple2Level[level].EventItemOwner = aIndex;

	this->m_IllusionTemple2Level[level].EventItemLevel = lpItem->m_Level;

	this->GCIllusionTemple2EventItemOwnerSend(&this->m_IllusionTemple2Level[level]);

	this->GCIllusionTemple2EventItemOwnerS8Send(&this->m_IllusionTemple2Level[level], aIndex, 0);

	this->NoticeSendToAll(&this->m_IllusionTemple2Level[level],0,gMessage.GetMessage(184),gObj[aIndex].Name);
}

void CIllusionTemple2::DestroyEventItem(int map,CMapItem* lpItem) // OK
{
	int level = GET_IT_LEVEL(map);
	...
	this->GCIllusionTemple2EventItemOwnerS8Send(&this->m_IllusionTemple2Level[level], this->m_IllusionTemple2Level[level].EventItemOwner, 1);

	this->m_IllusionTemple2Level[level].EventItemSerial = -1;
	...
	this->SetStoneStatue(&this->m_IllusionTemple2Level[level]);
}

void CIllusionTemple2::UserDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	int level = GET_IT_LEVEL(lpObj->Map);
	...
	if(this->GetUser(&this->m_IllusionTemple2Level[level],lpObj->Index) == 0)
	{
		return;
	}
	...
	ILLUSION_TEMPLE_USER* lpUser = this->GetUser(&this->m_IllusionTemple2Level[level],lpTarget->Index);
	...
	this->GCIllusionTemple2KillPointSend(lpUser->Index,lpUser->Point);
#else
	if (lpUser->Point >= 255)
	{
		return;
...
void CIllusionTemple2::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
#if (GAMESERVER_UPDATE <= 603)
	...
	if(this->GetMonster(&this->m_IllusionTemple2Level[level],lpObj->Index) == 0)
	{
		return;
	}
...
bool CIllusionTemple2::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	int level = GET_IT_LEVEL(lpObj->Map);
	...
	ILLUSION_TEMPLE_USER* lpUser1 = this->GetUser(&this->m_IllusionTemple2Level[level],lpObj->Index);
	...
	ILLUSION_TEMPLE_USER* lpUser2 = this->GetUser(&this->m_IllusionTemple2Level[level],lpTarget->Index);
...
void CIllusionTemple2::NoticeSendToAll(ILLUSION_TEMPLE_LEVEL* lpLevel,int type,char* message,...) // OK
{
	char buff[256];
...
void CIllusionTemple2::DataSendToAll(ILLUSION_TEMPLE_LEVEL* lpLevel,BYTE* lpMsg,int size) // OK
{
	for(int n=0;n < MAX_IT_USER;n++)
	{
		if(OBJECT_RANGE(lpLevel->User[n].Index) != 0)
...
void CIllusionTemple2::CGIllusionTemple2EnterRecv(PMSG_ILLUSION_TEMPLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	if(gServerInfo.m_IllusionTemple2Event == 0)
	{
		return;
	...
	ILLUSION_TEMPLE_LEVEL* lpLevel = &this->m_IllusionTemple2Level[lpMsg->level];
	...
	this->GCIllusionTemple2ChangeScenarioSend(lpLevel,0,aIndex);
}

void CIllusionTemple2::CGIllusionTemple2SkillRecv(PMSG_ILLUSION_TEMPLE_SKILL_RECV* lpMsg,int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	ILLUSION_TEMPLE_USER* lpUser = this->GetUser(&this->m_IllusionTemple2Level[level],aIndex);
	...
	{
		this->GCIllusionTemple2AddSkillSend(aIndex,bIndex,0,skill);
		return;
	}
	...
	switch(skill)
	{
		...
		case SKILL_SPURSUIT:
			result = gSkillManager.SkillOrderOfTracking(aIndex,this->m_IllusionTemple2Level[level].EventItemOwner);
			break;
	...
	else
	{
		...
		this->GCIllusionTemple2KillPointSend(aIndex,lpUser->Point);
		this->GCIllusionTemple2AddSkillSend(aIndex,bIndex,1,skill);
	}
}

void CIllusionTemple2::CGIllusionTemple2ButtonClickRecv(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	if(lpUser->Team != this->m_IllusionTemple2Level[level].WinnerTeam)
	{
		return;
	}

	if((GetLargeRand()%10000) < gServerInfo.m_IllusionTemple2RewardFenrirRate)
	{
	...
	if (this->m_IllusionTemple2Reward[level][0] > 0 || this->m_IllusionTemple2Reward[level][1] > 0 || this->m_IllusionTemple2Reward[level][2] > 0)
	{
		//Reward
		GDSetCoinSend(lpUser->Index, this->m_IllusionTemple2Reward[level][0], this->m_IllusionTemple2Reward[level][1], this->m_IllusionTemple2Reward[level][2],"IT");
		
		GCFireworksSend(&gObj[lpUser->Index],gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
	}
}

void CIllusionTemple2::GCIllusionTemple2BattleInfoSend(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
#if GAMESERVER_UPDATE <= 603
	for(int n=0;n < MAX_IT_USER;n++)
	{
...
void CIllusionTemple2::GCIllusionTemple2AddSkillSend(int aIndex,int result,int skill,int bIndex) // OK
{
...
void CIllusionTemple2::GCIllusionTemple2RewardScoreSend(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
#if GAMESERVER_UPDATE <= 603
	BYTE send[256];
...
//		GDRankingIllusionTemple2SaveSend(lpLevel->User[n].Index,lpLevel->User[n].Score);
...
void CIllusionTemple2::GCIllusionTemple2KillPointSend(int aIndex,int point) // OK
{
#if GAMESERVER_UPDATE <= 603
...
void CIllusionTemple2::GCIllusionTemple2DelSkillSend(int aIndex,int skill) // OK
{
#if (GAMESERVER_UPDATE <= 603)
...
void CIllusionTemple2::GCIllusionTemple2EventItemOwnerSend(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
#if (GAMESERVER_UPDATE <= 603)
...
void CIllusionTemple2::GCIllusionTemple2EventItemOwnerS8Send(ILLUSION_TEMPLE_LEVEL* lpLevel, int Index, int byGet) // OK
{
#if (GAMESERVER_UPDATE >= 803)
	PMSG_ILLUSION_TEMPLE_EVENT_ITEM_OWNER_SEND pMsg;
...
void CIllusionTemple2::GCIllusionTemple2ChangeScenarioSend(ILLUSION_TEMPLE_LEVEL* lpLevel,int state,int aIndex) // OK
{
#if GAMESERVER_UPDATE <= 603
...
void CIllusionTemple2::GCIllusionTemple2StoneInfoSend(ILLUSION_TEMPLE_LEVEL* lpLevel,int type,int state) // OK
{
#if (GAMESERVER_UPDATE >= 803)
	for(int n=0;n < MAX_IT_USER;n++)
	{
