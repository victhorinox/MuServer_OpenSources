...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	{
		GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GetMessage(549)); //- CustomNpcMove add Item Requeriment
		return 1;
	}
	...
	switch(lpNpc->Class)
	{
		...
		case 241:
			...
			break;
		case 784:
			this->NpcGuildWarehouse(lpNpc,lpObj);
			break;
		case 249:
		...
		case 579:
			...
			break;
		case 780:
			this->NpcCustomArena(lpNpc,lpObj);
			break;
#endif
...
{
	if (gServerInfo.m_CustomBloodCastleWindow == 1)
	{
		if(lpObj->ChangeUp < 2)
		{
			GCChatTargetSend(lpObj,lpNpc->Index,"Somente personagens nível mestre podem participar.");
			return;
		}

		if(gBloodCastle.GetEnterEnabled(0) == 0 || gBloodCastle.GetEnterEnabled(1) == 0 || gBloodCastle.GetEnterEnabled(2) == 0 ||gBloodCastle.GetEnterEnabled(3) == 0 || gBloodCastle.GetEnterEnabled(4) == 0 || gBloodCastle.GetEnterEnabled(5) == 0 || gBloodCastle.GetEnterEnabled(6) == 0 || gBloodCastle.GetEnterEnabled(7) == 0)
		{
			GCChatTargetSend(lpObj,lpNpc->Index,"O Blood Castle está fechado, tente novamente mais tarde.");
			return;
		}
...
void CNpcTalk::NpcGuildWarehouse(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
#if(GAMESERVER_TYPE==1)
	if(lpObj->GuildNumber > 0)
	{
...
void CNpcTalk::NpcElfBuffer(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	//- Fix Elf Buffer to bless only once every time
	if(gEffectManager.CheckEffect(lpObj,EFFECT_ELF_BUFFER) != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You are already blessed");
		return;
	}
...
void CNpcTalk::NpcCustomArena(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
#if(GAMESERVER_TYPE==0)
	//- Add Custom Arena Npc
	if (gServerInfo.m_CustomArenaNpcEnable == 1)
	{
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex0) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex0);
		}
		//-
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex1) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex1);
		}
		//-
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex2) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex2);
		}
		//-
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex3) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex3);
		}
		//-
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex4) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex4);
		}
		//-
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex5) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex5);
		}
		//-
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex6) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex6);
		}
		//-
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex7) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex7);
		}
		//-
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex8) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex8);
		}
		//-
		if (gCustomArena.CheckEnterEnabled(lpObj,gServerInfo.m_CustomArenaGateIndex9) != 0)
		{
			gObjMoveGate(lpObj->Index,gServerInfo.m_CustomArenaGateIndex9);
		}
		//--
		else
		{
			GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GetMessage(541));
		}
	}
#endif 
}
