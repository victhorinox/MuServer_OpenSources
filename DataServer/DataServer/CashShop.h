...
public:
	...
	//jewel bank
	void GDBankExReqPoint(BANKEX_GD_REQ_POINT * aRecv, int aIndex);
	void BankExSavePoint(BANKEX_GD_SAVE_POINT * aRecv);
	bool BankExReqPoint(char * AccountID, BANKEX_DG_GET_POINT * Result);
	bool BankExReq(char* AccountID, BANKEX_DG_GET_POINT * Result);
	char BankExTable[32];
	char BankExColumn[32];
	char BankExAccount[32];
};
